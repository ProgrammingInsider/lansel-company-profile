  ...
  return (
    <div className='grid md:grid-cols-5 text-center md:text-left w-11/12 mx-auto max-w-6xl py-20 gap-5'>
       <div className='flex flex-col items-center md:items-end text-center md:text-left w-11/12 mx-auto max-w-6xl col-span-3 md:order-1'>
          <h1 className='sectionName w-80 md:mr-5'>Why Choose Lansel Trading?</h1>
          <h1 className='sectionHeader mb-10 md:text-right'>Cornerstones of Our Business</h1>
          <div>
            <div className='flex flex-col items-center gap-5'>
              {
                whyuslist.map((item) => (
                  <div key={item.id} className='grid grid-cols-5 items-center p-3 rounded-xl sectionBg'>
                    <div className='col-span-1'>
                      <h1 className='primaryBg text-white font-bold text-6xl p-5 pt-7 rounded-full flex justify-center items-center w-16 h-16'>{item.id}</h1>
                    </div>
                    <div className='col-span-4'>
                      <h3 className='text-2xl primaryText text-left'>{item.title}</h3>
